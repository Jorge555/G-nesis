<?php
  ...
  function  insertarU($a,$b,$c,$d,$e){
    //  echo "<script> alert('Conexion exitosa $a $b $c $d $e'); </script>";
    $sql="INSERT INTO usuarios(`USUARIO`,`PASS`,`ROL`,`PERSONAL`,`ACTIVO`)
      VALUES  ('$a','$b','$c','$d','$e')";
    $consulta= conectar()->query($sql);
    return  $consulta;
  }
  ...
  function BuscarU($a){
    $sql ="SELECT personal.ID,usuarios.USUARIO,usuarios.PASS,rol.ID,distrito.ID,usuarios.ACTIVO FROM usuarios
     INNER JOIN personal on personal.ID= usuarios.PERSONAL
     INNER JOIN rol on rol.ID=usuarios.ROL
    ...
    $filas=$consulta->num_rows;
    while($fila=mysqli_fetch_array($consulta)){
      $Usuario[0]["PERSONAL"]=$fila[0];
      $Usuario[0]["USER"]=$fila[1];
      $Usuario[0]["PASS"]=$fila[2];
      $Usuario[0]["ROL"]=$fila[3];
      $Usuario[0]["DIST"]=$fila[4];
      $Usuario[0]["ACTIVO"]=$fila[5];
      $Usuario[0]["ID"]=$a;
    }
    $consulta->free();
    return $Usuario;
  ...
  function ActualizarU($a,$b,$c,$e,$dist,$f){
    $sql ="UPDATE usuarios INNER JOIN personal ON usuarios.PERSONAL=personal.ID
      INNER JOIN distrito ON personal.DISTRITO=distrito.ID SET
  ...
  function Distrito($a){
    ...
    $sql ="SELECT distrito.ID FROM `personal` 
  ...
  function TipoU($a){
    $sql ="SELECT usuarios.ROL FROM `personal` 
    INNER JOIN usuarios on personal.ID= usuarios.PERSONAL 
    INNER JOIN distrito ON personal.DISTRITO=distrito.ID 
    WHERE usuarios.USUARIO='$a'";
    $consulta= conectar()->query($sql);
